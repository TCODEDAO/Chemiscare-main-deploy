...
function CreateThreadComponent({ handleHideEditorThread }) {
    return (
        <div className='h-screen w-screen fixed z-999 bg-[#ffffff46]' onDoubleClick={(e) => {
            e.stopPropagation()
            handleHideEditorThread()
        }}>
            <div className='w-[50%] h-[54%] min-w-[630px] min-h-[408px] fixed z-9999 bg-slate-50 rounded-[40px] overflow-hidden   top-[20%] left-[50%] translate-x-[-50%] transition-all duration-200 show-dialog' onDoubleClick={(e) => e.stopPropagation()}>
                <div>
                ...
                <div className='threadInput '>
                    <input type="text" className="inputText outline-none" required />
                ...
                <div className="flex justify-center items-center relative top-[66%] left-[28%]">
                    ...
                    <button className='mx-4 px-[36px] py-[14px] bg-[#3498db] hover:bg-[#2ecc71] transition-all duration-200 rounded-[20px] text-white'>Tạo</button>
                </div>
            </div>
        </div>
    )
